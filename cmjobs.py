...
def main():
  ...
  for proc in processes:
    ...
    if iTime <= 3: tRemain = "unknown"
      ...
      tRunning = time.time() - os.path.getmtime(pcwd+"/params.out")
  ...
  # finish processes matching the pattern
  if function=="finish":
...
if __name__ == '__main__':
  if len(sys.argv)>1: 
    ...
    if function not in ["list", "kill", "finish"]:
      raise ValueError(f"Unknown option '{function}'. Valid options are 'list', 'finish' and 'kill'.")
    else:
      do_filt = True
  if len(sys.argv)>2: 
    pattern = sys.argv[2]
